...
let make = (~story: StoryData.story, ~index: int, ()) => {
  let renderIndex = () =>
    <aside className="StoryListItem_storyIndex"> {React.string(string_of_int(index + 1))} </aside>
  let renderTitle = () => {
    let content = React.string(story.title)
    <header className="StoryListItem_storyTitle">
      {switch story.url {
      | Some(url) => <a href=url className="StoryListItem_link" target="_blank"> content </a>
      | None =>
  ...
  let renderByline = () =>
    <div className="StoryListItem_row StoryListItem_byline">
      <span className="StoryListItem_number">
        <b> {React.string(string_of_int(story.score))} </b> {React.string(" points")}
      </span>
  ...
  <div className="StoryListItem_itemRow"> {renderArticleButton()} {renderCommentsButton()} </div>
}
